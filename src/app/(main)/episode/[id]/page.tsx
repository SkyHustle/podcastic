...
const getEpisode = cache(async (id: string): Promise<Episode> => {
  const { data: episode, error } = await supabase.from('episodes').select('*').eq('id', parseInt(id)).single()
        ...
        <header className="flex flex-col">
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              <h1 className="mt-2 text-xl font-bold text-slate-900 sm:text-2xl md:text-4xl">{episode.title}</h1>
